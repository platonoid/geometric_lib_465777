...
# General discription
## File circly.py
- import math - Эта строка импортирует модуль math.Модуль math предоставляет доступ к математическим функциям и константам, таким как число Пи (math.pi).Без этой строки мы не могли бы использовать math.pi в нашем коде.

- def area(r): - Определяет функцию под названием area, которая принимает один аргумент r (радиус круга). r - это переменная,  которая хранит значение радиуса, переданного в функцию.

- return math.pi * r * r - Эта строка  выполняет вычисление площади круга. math.pi - это константа, представляющая число Пи. r * r - это радиус в квадрате. return возвращает результат вычисления  из функции area.

- def perimeter(r): - Определяет функцию под названием perimeter, которая принимает один аргумент r (радиус круга).
...
## File square.py
- def area(a): - Определяет функцию с именем area, которая принимает один аргумент a. a - это переменная, которая будет хранить значение, переданное в функцию
...
## Параметры
* **a**,**b**,**c** (int),(int),(int) стороны треугольника
## Возвращает значение 
* (int) Площадь трегольника
## Примеры
* >area(10,8,6)
  > >10
...
# History
- Tue Mar 30 18:02:23 2021 +0300 | b5b0fae | Daniil.K |L-04: Update docs for calculate.py
- Tue Mar 30 17:57:42 2021 +0300 | d76db2a | Daniil.K |L-04: Add calculate.py
